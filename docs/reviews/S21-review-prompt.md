...
A different model implemented step S21 of a build plan. Your job is to find what is
...
`make timeline` prints one JSON object of what actually happened in the last run -
...
docs/evidence/s21-checkpoint.log
docs/evidence/timeline.log
docs/todo.md
...
6. **Run `scripts/checks/S21.sh` yourself.** Does it pass *now*, on the current working
   tree?
...
Write `docs/reviews/S21-findings.md`:
    ...
    <did S21.sh pass on a clean run? does it actually assert the step's goal?
    one paragraph.>
