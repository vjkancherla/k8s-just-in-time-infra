...
A different model implemented step S17 of a build plan. Your job is to find what is
...
the demo, and the acceptance tests from the design note.

## What the step was allowed to touch

app/kustomize/overlays/voting-a/kustomization.yaml
app/kustomize/overlays/voting-b/kustomization.yaml
scripts/verify-jit.sh
Makefile
README.md
scripts/checks/S17.sh

## What to read

- `docs/design/jit-infra-poc.md` - the design this must conform to
- `docs/build-plan.md` - step S17, its checkpoint, its gate
- `git diff a82a6a9..e7ef515` - what was actually done
- `scripts/checks/S17.sh` - the assertion that passed
...
6. **Run `scripts/checks/S17.sh` yourself.** Does it pass *now*, on the current working
   tree?
...
Write `docs/reviews/S17-findings.md`:
    ...
    <did S17.sh pass on a clean run? does it actually assert the step's goal?
    one paragraph.>
