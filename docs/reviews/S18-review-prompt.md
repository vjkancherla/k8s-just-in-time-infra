...
A different model implemented step S18 of a build plan. Your job is to find what is
...
every action the console can take is a single make target, and the console's
...
Makefile
scripts/state.sh (new)
scripts/demo-up.sh (new)
docs/evidence/ (the targets' logs, and s18-final-gate.log)

## What to read

- `docs/design/jit-infra-poc.md` - the design this must conform to
- `docs/build-plan.md` - step S18, its checkpoint, its gate
- `git diff 13c2502..5a1b59c` - what was actually done
- `scripts/checks/S18.sh` - the assertion that passed
...
6. **Run `scripts/checks/S18.sh` yourself.** Does it pass *now*, on the current working
   tree?
...
Write `docs/reviews/S18-findings.md`:
    ...
    <did S18.sh pass on a clean run? does it actually assert the step's goal?
    one paragraph.>
