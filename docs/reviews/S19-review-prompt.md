...
A different model implemented step S19 of a build plan. Your job is to find what is
...
the page is one file with no build step, every button it offers is a make target
`make targets` lists, it calls only the console's own endpoints, and every field it reads
out of the read model exists in `make state`'s document - so the page cannot drift from
the CLI.
...
- `docs/jit-infra-poc.md` - the design this must conform to
- `docs/build-plan.md` - step S19, its checkpoint, its gate
- `git diff 205aedc..7210a8d` - what was actually done
- `scripts/checks/S19.sh` - the assertion that passed
...
6. **Run `scripts/checks/S19.sh` yourself.** Does it pass *now*, on the current working
   tree?
...
Write `docs/reviews/S19-findings.md`:
    ...
    <did S19.sh pass on a clean run? does it actually assert the step's goal?
    one paragraph.>
