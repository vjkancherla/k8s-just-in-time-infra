...
J6  PASS after the 2m window postgres+pgadmin were swept in 220s (container, Secret, Service, EndpointSlice) while redis stayed up; postgres data volume removed
J7  PASS vote gone: redis kept Ready + referencedBy [voting-app-worker] (last-reference rule) while postgres Orphaned
J8  PASS deleted ns voting-b: its 3 containers and claims were gone in 30s with no TTL armed (hard path), voting-a's 3 claims still Ready
