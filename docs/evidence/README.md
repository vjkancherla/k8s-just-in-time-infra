# Evidence

Verbatim copies of the run logs and probe scripts that the step records in `docs/todo.md` and
`memory-bank/` cite. They used to live in `/tmp`, which meant the citations pointed at files that a
reboot would delete — and two of them had already been lost that way before S17. Moving them here
keeps every reference in `docs/` and `memory-bank/` resolvable, and lets a reader check the claims
rather than take them on trust.

Nothing here is executed by any workflow. These are artifacts.

## S17

| File | What it is |
|---|---|
| `s17-run.log` | The complete `bash scripts/checks/S17.sh` run that passed: `PASS: J1-J11 all PASS`, `PASS: ===== 17 PASS, 0 FAIL ===== in voting-a` |
| `s17-green-run.log` | Copy of the same run, kept separately because S17's earlier evidence was overwritten mid-run |
| `verify-jit-green.md` | `.workflow/verify-jit.md` from that run — `===== 11 PASS, 0 FAIL =====` |
| `s17-run-j11fail.log` | The first complete run, which failed at J11: MinIO answered **403 SignatureDoesNotMatch** because the S3 prefix was signed as `ns/` |
| `verify-jit-j11fail.md` | The suite report from that run, ending `10 PASS, 1 FAIL` |
| `s17-run-j11-nopass.log` | The second complete run, which failed the gate although J11 passed — J11 had no `pass` line, so a passing J11 printed nothing |
| `verify-jit-j11-nopass.md` | The suite report from that run: `10 PASS, 0 FAIL` and no J11 line |
| `race-test.sh` / `race-test.log` | Three clean-slate `voting-a` deploys, each requiring three distinct IPs and `count: 3` — the evidence that J1's duplicate-IP race is fixed rather than lucky. 3/3 OK |
| `block-race-test.sh` / `block-race.log` | Two iterations of `overlays/voting-a` and `overlays/voting-b` applied back to back, looking for the per-namespace lock failing to serialise the shared `jit-ipam` ConfigMap. Did **not** reproduce in two attempts |
| `leak-probe.sh` / `leak-probe.log` | v1 of the retry-path leak probe. It restarted the runner as soon as the phase went `Deleting`, so the in-flight destroy succeeded and the ledger looked clean — it tested nothing. Kept as the example of a probe that passes for the wrong reason |
| `leak-probe2.sh` / `leak-probe2.log` | v2, which holds the runner down through two failed retries. It proved the retry path leaks the block: claim and container gone, ledger still `{"hatch-leak": ... "count": 1}` |
| `leak-probe3.sh` / `leak-probe3.log` | v3, the same scenario after the fix. Same script body, so the two logs are comparable: `LEDGER AFTER: {"voting-a": ... "count": 3}` — no `hatch-leak` entry |
| `leak-probe3-controller.log` | The controller log for that run, showing the order that makes the release exactly-once: the retry branch destroys, deletes the claim, then `jit-ipam INFO Released IP block for namespace hatch-leak`, and only then does the delete handler run and skip (`already swept; not releasing its IP block twice`) |
| `s17-jitdown-bounce.sh` / `s17-jitdown-bounce.log` | The review's concern 1 end to end, and the tenant-recovery path: `jit-ipam` absent after `jit-down`, `jit-up` re-runnable, no claims left over, then a Deployment touch re-creates the claims and the ledger is rebuilt as `{"voting-a": ... "count": 3}` — ending `17 PASS, 0 FAIL` in `voting-a` |
| `s17-jitdown-bounce-race.log` | The second run of that script, which **failed** and found a defect: `jit-down` deleted the claims with `--wait=false` and then removed the runner, so they stayed Terminating (`must be 0): 3` for the full two minutes), came back *Ready* with no container behind them, and blocked the tenant's re-provisioning (`containers up: 0 of 3`, R2 FAIL). `jit-down` now waits for the claims to go. Kept because the green run alone would not have proved it |
| `s17-rchecks-negative-options.log` | The R-checks pointed at a dead app (`VOTE_URL`/`RESULT_URL` unreachable), which is how `app/scripts/verify.sh` was found to abort checks with `OPTS[0]: unbound variable` and to let R5 pass vacuously on an empty option name. Now `10 PASS, 7 FAIL` with each failure carrying its own reason and no shell noise |
| `s17-cold-path.sh` / `s17-cold-path.log` | The build plan's last Done item run for real. It got through destroy → deploy → jit-down → jit-up → `make all` deploying into `voting-a` (the alignment works: three claims Ready, vote rolled out) and then **stopped at a blocker the cold path had hidden**: `init-db` failed with `FATAL: password authentication failed for user "postgres"`, because the postgres data volume survives the container sweep while the controller generates a fresh password for the new stack. This is the run that raised the question |
| `s17-cold-path-green.log` | The same script, phase `full`, after the question was answered (**the volume goes with its container**). Step 3 is the fix in one line: `jit-volumes=[voting-a-postgres-postgres-data ]` before `make jit-down`, `jit-volumes=[]` after it. The run ends cold and green — `make all` = `17 PASS, 0 FAIL`, `make jit-verify` = `11 PASS, 0 FAIL`, final state three claims Ready with the ledger rebuilt as `{"voting-a": {"offset": 0, "count": 3}}`. This is the build plan's last Done item satisfied |
| `s17-cold-path-gaps.log` | The first cold run, which stopped at both gaps: `jit-up` failing on `ImagePullBackOff` because the fresh cluster had no controller image, and `jit-down` exiting 1 on a cluster with no CRD — a bug in the wait added the same day |
| `s17-cold-path-order.log` | The second cold run: `make all` green (17 PASS) in `default`, then `make jit-verify` refusing to start — `PREREQ FAIL an Ingress outside voting-a claims vote.localhost (default)`. The same host cannot be owned by two demo namespaces |
| `s17-final-gate.log` | `bash scripts/checks/S17.sh` re-run on the final code, after the fix: the Makefile assertions, `make jit-verify` = `11 PASS, 0 FAIL`, `make verify` = `17 PASS, 0 FAIL` in `voting-a`. The log ends on the script's own final PASS line, which under `set -euo pipefail` is reachable only on success |
| `clean-slate.sh` | The reset helper every probe above calls: claims, JIT containers, volumes, the demo namespaces and the IPAM ledger |
| `restore-env.sh` | Puts the demo back after the probes (voting-a deployed, ledger reset) |

## S16

Cited by `docs/todo.md`'s S16 record and `memory-bank/journal/2026-10-09.md`.

| File | What it is |
|---|---|
| `s16-run1-keep.log`, `s16-run2.log`, `s16-run3.log` | `bash scripts/checks/S16.sh` exit 0 on three consecutive runs |
| `s16-final.log` | The final S16 run before the review |
| `s16-postfix2.log` | The re-run after the review's concerns were fixed |
| `s16-negative.log`, `s16-negative2.log` | The negative test: the helpers pointed at non-existent containers, so the guarded checks FAIL with a readable reason instead of comparing two empty reads |
| `s16-premigration-tally.txt` | The empty pre-migration read, kept as the reason the helpers are guarded |

## Reproducing

The scripts are preserved exactly as they ran, so their internal paths still say `/tmp/...`. To run
one, put it and its siblings in `/tmp` first, or edit the paths at the top. They expect the JIT stack
up (`make jit-up`) and a `voting-a`/`voting-b` demo to work on; they create and destroy containers.
