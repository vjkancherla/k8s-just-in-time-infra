...
|----------|------|----------|--------|------------|
| vote | Deployment | 1 | HTTP `/healthz` | SECRET_KEY, REDIS_URL; JIT claims for redis/postgres/pgadmin |
| worker | Deployment | 1 | Exec `--healthcheck` | Redis URL, PG credentials; JIT claim for redis |
| result | Deployment | 1 | HTTP `/healthz` | PG credentials, initContainer |

**There is no stateful workload in the cluster.** Redis, Postgres and pgAdmin are provisioned
*outside* it by the JIT controller and runner, and the pods consume the controller-written
`jit-redis`, `jit-postgres` and `jit-pgadmin` Services. No StatefulSet, no PersistentVolumeClaim
(build-plan S15).
...
Two Secrets carry credentials, and neither is a committed credential:

| Secret | Holds | Written by |
|--------|-------|------------|
| `voting-app-secret-key` | `SECRET_KEY` (Flask session key) | Kustomize `secretGenerator`, stable name, committed dev value |
| `jit-postgres` | the postgres module outputs plus `POSTGRES_PASSWORD` | the controller, from the password it generates per namespace |
| `jit-redis` | the redis module outputs (`address`, `port`, `url`) | the controller |

The pods read `PGPASSWORD` from `jit-postgres` via `secretKeyRef`, and `REDIS_URL` points at the
`jit-redis` Service. `postgres-secret.env` and the `openssl` step that generated it are gone.
...
| Area | Note |
|------|------|
| **RESOLVED — POSTGRES_DB mismatch** | The database is `voting` everywhere: the postgres module defaults `postgres_db = "voting"` and every pod sets `PGDATABASE=voting`. The old `votingdb` placeholder is gone (build-plan S15). |
...
3. Checks the JIT stack is up (the `InfraClaim` CRD must exist — the app has no stateful workload of its own)
4. Applies Kustomize: `kubectl apply -k kustomize/` (or `overlays/registry/` in registry mode)
5. Waits for the three application workloads to become ready (`kubectl rollout status`)
...
- Uses a `sed`-based image override for R12; the registry overlay's own build is asserted by `scripts/checks/S15.sh`
...
| `make destroy` | `scripts/cleanup.sh` (full) | Delete resources + k3d cluster |
| `make status` | `kubectl get pods,svc,ingress` | Cluster status |

**Env-var passthrough:** `CLUSTER`, `REGISTRY`, `REGISTRY_PORT`, `DELETE_CLUSTER`
...
| Gap | Impact | Location |
|-----|--------|----------|
| **RESOLVED — POSTGRES_DB mismatch** | Database is `voting` everywhere now; the postgres module defaults `postgres_db = "voting"` (build-plan S15) |
...
Ignores: `__pycache__/`, `*.py[cod]`, `.venv/`, `*.log`, `helm/voting-app/charts/`.
...
| **Deploy idempotency** | `k3d cluster create` only runs if the cluster is absent, and the controller reuses the password stored in `jit-postgres`, so a re-deploy does not rotate credentials |
