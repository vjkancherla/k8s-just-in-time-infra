...
Postgres credentials are not generated here any more. The controller generates the password per namespace
and writes it into the `jit-postgres` Secret; the Flask `SECRET_KEY` lives in its own
`voting-app-secret-key` Secret. There is no `postgres-secret.env` (build-plan S15).
...
**Idempotency:** the cluster is created only if absent, and the Postgres password is generated once per
namespace and then reused from the `jit-postgres` Secret, so credentials stay stable across re-deploys and a
Postgres container that already holds data is never asked to change its password.
...
- Uses a `sed`-based image override for R12 rather than building the registry overlay. R12 is deliberately
  out of scope for rework (build-plan S16 says do not change R1, R3-R7 or R12-R15), so the overlay's own
  build is asserted by `scripts/checks/S15.sh` instead.
...
cluster is removed only when explicitly requested, so a plain run is safe to repeat. There is no generated
secret file to clean up any more — Postgres credentials live in the `jit-postgres` Secret inside the cluster.
...
3. **Optionally delete the k3d cluster** (`DELETE_CLUSTER=1`), which also removes the cluster-internal
   registry if one was created and any container the runner provisioned on its network.
...
| `KUSTOMIZE_DIR` | deploy.sh, verify.sh | `./kustomize` | Kustomize entry point (a wrapper over `base/`); overlays live in `overlays/` |
