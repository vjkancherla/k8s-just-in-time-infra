...
- **redis**: pure FIFO queue (`votes` list). No volume, no AOF — votes are lost if the Redis *container* restarts. Runs as `default-redis-redis` on the k3d network, provisioned by the JIT controller.
- **worker**: drains Redis, upserts one row per `voter_id` into Postgres (idempotent).
- **postgres**: stores the `votes` table in a named Docker volume. Runs as `default-postgres-postgres`; the password is generated per namespace and lives in the controller-written `jit-postgres` Secret. No StatefulSet, no PVC.
...
> **The JIT stack must already be up**: MinIO, the `jit-runner` container, the
> `InfraClaim` CRD and the `jit-controller` Deployment (`deploy/minio.sh`,
> `deploy/runner.sh`, `deploy/controller.yaml`). Redis, Postgres and pgAdmin are
> provisioned by it, out of cluster. `openssl` and `kustomize/postgres-secret.env`
> are no longer used — the controller generates and stores the password
> (build-plan S15).
...
3. Applies Kustomize and waits for the three app Deployments to become ready.
   It fails fast if the `InfraClaim` CRD is missing: the pods stay in `Init`
   until the controller has written the `jit-redis` / `jit-postgres` Services
   and Secrets (build-plan S15).
...
Confirm the three app workloads are `Ready` and the three JIT containers are up:

```bash
# Pods
kubectl get pods

# Expected: vote, worker, result each 1/1 Ready
docker ps --filter 'name=^default-'      # default-redis-redis, default-postgres-postgres, default-pgadmin-pgadmin
kubectl get infraclaims                  # default-redis, default-postgres, default-pgadmin: phase Ready
```
...
> ```bash
> docker exec default-redis-redis redis-cli ping            # -> PONG
> docker exec default-postgres-postgres pg_isready -U postgres     # -> accepting connections
> ```
...
before=$(docker exec default-redis-redis redis-cli LLEN votes)
curl -sk -X POST https://vote.localhost:8082/vote -d "choice=Cats" -c /tmp/cj-r2.txt -o /dev/null -w '%{http_code}'
after=$(docker exec default-redis-redis redis-cli LLEN votes)
...
sleep 3
docker exec default-postgres-postgres psql -U postgres -d voting -t -A -c 'SELECT COUNT(*) FROM votes'
```
...
```bash
docker exec default-redis-redis redis-cli RPUSH votes '{"voter_id":"manual-r4","choice":"Cats","timestamp":1700000000}'
docker exec default-redis-redis redis-cli RPUSH votes '{"voter_id":"manual-r4","choice":"Cats","timestamp":1700000000}'
sleep 3
docker exec default-postgres-postgres psql -U postgres -d voting -t -A -c "SELECT COUNT(*) FROM votes WHERE voter_id='manual-r4'"
```
...
```bash
docker exec default-postgres-postgres psql -U postgres -d voting -c '\d votes'
```
...
docker exec default-redis-redis redis-cli LLEN votes          # > 0 (queued)
kubectl scale deployment/voting-app-worker --replicas=1
kubectl rollout status deployment/voting-app-worker --timeout=120s
sleep 5
docker exec default-postgres-postgres psql -U postgres -d voting -t -A -c 'SELECT COUNT(*) FROM votes'
docker exec default-redis-redis redis-cli LLEN votes          # expect 0 (drained)
```
...
```bash
before=$(docker exec default-postgres-postgres psql -U postgres -d voting -t -A -c 'SELECT COUNT(*) FROM votes')
docker restart default-postgres-postgres
until docker exec default-postgres-postgres pg_isready -U postgres >/dev/null 2>&1; do sleep 2; done
after=$(docker exec default-postgres-postgres psql -U postgres -d voting -t -A -c 'SELECT COUNT(*) FROM votes')
echo "before=$before after=$after"
```

✅ **Pass:** `before == after` — the named Docker volume (`default-postgres-data`) preserves the tally across a container restart.

> **Timing note:** the `until` loop above waits for Postgres to accept connections again; give the result service a further moment to reconnect before concluding.

---

## 8. Platform / Infrastructure Testing (R10–R17)

### R10 — Liveness + readiness probes on all 3 workloads

```bash
kubectl get deploy -n default -o json | jq '
...
✅ **Pass:** `liveness=3 readiness=3`.

### R11 — Resource requests AND limits on all 3
...
### R17 — Credentials from the JIT outputs Secret, not committed YAML

```bash
grep -rn 'POSTGRES_PASSWORD' ./kustomize --include='*.yaml'   # expect NO matches
kubectl get secret jit-postgres -o jsonpath='{.data.POSTGRES_PASSWORD}' | wc -c
```

✅ **Pass:** no literal password in the manifests, and the Secret `jit-postgres` holds `POSTGRES_PASSWORD`.

> The controller generates the password once per namespace, reuses the value stored in the Secret when re-provisioning (so a redeploy does not rotate it), and writes it there. The app pods read it with `secretKeyRef`; `kustomize/postgres-secret.env` is gone (build-plan S15).
...
By default the app enforces one vote per browser so R3 passes. `make demo-up` deploys with `ALLOW_MULTIPLE_VOTES=false`, verifies all R-checks, then automatically enables demo mode (`ALLOW_MULTIPLE_VOTES=true`) so you can click repeatedly and see every click counted. `make demo-redeploy` also re-enables demo mode after re-applying the overlay.

If you deployed another way and want to enable demo mode manually:
...
Set it back to `false` before running `make verify`, because R3 expects a re-vote from the same browser to update the existing row rather than create a new one.
